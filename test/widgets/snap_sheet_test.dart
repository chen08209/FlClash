...
void main() {
  ...
  testWidgets('a second drag while the sheet is still expanding finishes the '
      'expansion and scrolls the content with what is left', (tester) async {
    await openSheet(tester);
    final expanded = _mobile.height * (1 - snapSheetDetents.last);

    await tester.fling(content(), const Offset(0, -100), 1000);
    await tester.pump(const Duration(milliseconds: 100));
    final midway = sheetTop(tester);
    expect(midway, greaterThan(expanded + 1));
    expect(contentOffset(tester), 0);
    ...
    expect(sheetTop(tester), closeTo(expanded, 1));
    expect(contentOffset(tester), closeTo(300 - (midway - expanded), 30));
