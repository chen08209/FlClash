...
void main() {
  ...
  group('EditProxyProvidersView', () {
    ...
    testWidgets('the add picker lists the subscription first in one list', (
      tester,
    ) async {
      container = buildContainer(_group(use: const []));
      ...
      expect(find.text(currentAppLocalizations.proxyProviders), findsOneWidget);
      expect(top('provider-a'), lessThan(top('p')));
      expect(top('provider-a'), lessThan(top('Shared nodes')));
      expect(find.text(currentAppLocalizations.profile), findsNothing);
    });

    testWidgets('the add picker shows a name the subscription has once', (
      tester,
    ) async {
      container = buildContainer(
        _group(use: const []),
      ...
      expect(top('Shared nodes'), lessThan(top('p')));
    });
