...
| `select_proxy` | Basic | Existing member of a visible `Selector` group only; a current profile is required. Calls the app's selection action, checks its actual result, then re-reads the core selection and checks UI state. Claims per-group intent before looking up the group; newer manual/MCP intent, profile transitions (including A→B→A), core disconnection and config replacement supersede it. Failed overlapping changes restore the last confirmed selection rather than an unconfirmed optimistic choice. The validated core lookup supplies the initial rollback baseline, including when no saved override exists. Uses existing app-managed persistence, but **never** calls connection close/reset maintenance from this basic tool, even when selecting the current member. Manual UI selection retains its existing connection-maintenance preference. Persistence is asynchronous under the existing app mechanism, not a transactional disk guarantee. |
| `test_delays` | Basic | 1–8 distinct existing nodes in visible groups. Uses **only** the application's configured test URL, at most two concurrent probes, updates the app's delay cache only while the run is current. Profile changes, core disconnection (even followed by reconnection), explicit cancellation, and every actual app configuration replacement (including same-profile subscription reloads) cancel pending results and remaining probes. `-1` is the core's failed measurement; missing core responses are errors, not fake measurements. |
...
Explicit UI/MCP delay tests own their guarded RPC results. The Go wrapper suppresses their duplicate native hook events by URL/name while probes are in flight, including overlapping probes; the pinned core invokes that hook synchronously before returning. Unrelated background health events still update the cache. A background event sharing the exact URL/name of an active explicit probe is also suppressed until those probes settle.
...
- After initialization or token-rotation storage failure, use the local **Retry** button. Retry is serialized with other settings operations and stays disabled/advanced-locked afterward; explicitly enable again. A retry retains the pending replacement token rather than loading the revoked token from the failed rotation. Storage failures are not a transactional revocation guarantee across an application restart.
- Maximum 8 sessions; ten-minute idle expiry (one-minute sweep). Maximum 8 admitted requests and one ordinary in-flight request per session, with bounded cancellation/session-close traffic admitted alongside it; one active tool invocation globally. Busy clients receive 429 or `busy`, rather than building an unbounded work queue.
...
Tests live in `test/ai_mcp/`: real isolated loopback initialize/list/call/cancel/DELETE and auth/Host/Origin/admission controls, held real-backend preflights and delay batches, actual applyProfile replacement, SQLite watch echoes and overlap result-order permutations, UI countdown/manual consent/relock/copy/Retry/disposal. `core/delay_events_test.go` exercises the pinned native adapter and overlapping hook suppression; manager tests retain background event delivery. They do not change host proxy settings, registry, or the installed client. Application stack ownership is covered by `test/application_test.dart`.
