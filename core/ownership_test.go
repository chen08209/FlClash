...
func TestCollectReclaimTargetsWalksWithoutFollowingSymlinks(t *testing.T) {
	...
	for _, path := range collectReclaimTargets(homeDir, uid) {
		if path == filepath.Join(outside, "secret") {
...
func TestReclaimEntryVerifiesTheInodeItChowns(t *testing.T) {
	...
	if ok, err := reclaimEntry(file, uid, gid); ok || err != nil {
	...
	if ok, err := reclaimEntry(hardLink, foreign, gid); ok || err != nil {
	...
	if _, err := reclaimEntry(symLink, foreign, gid); !errors.Is(err, syscall.ELOOP) {
	...
	if ok, err := reclaimEntry(fifo, foreign, gid); ok || err != nil {
		t.Errorf("reclaimEntry = (%v, %v), want a fifo skipped without blocking", ok, err)
	}

	if _, err := reclaimEntry(filepath.Join(homeDir, "missing"), foreign, gid); !errors.Is(err, fs.ErrNotExist) {
	...
	if _, err := reclaimEntry(plain, foreign, gid); err == nil {
