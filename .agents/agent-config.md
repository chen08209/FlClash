...
- `.claude/agents/*.md`: subagent definitions and the single source of truth for roles; `.codex/agents/*.toml` is
  generated from them by `tool/sync_agents.sh`, and `--check` runs in pre-commit. Subagents are separate contexts that
  do not inherit the parent's loaded instructions, so every always-on rule a subagent must not break is restated in
  full inside each definition's constraints. Roles, contracts, and the pipeline are described in
  `.agents/agents.md`.
