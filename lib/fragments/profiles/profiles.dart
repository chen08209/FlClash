...
class _ProfilesFragmentState extends State<ProfilesFragment> {
  ...
  void initState() {
    super.initState();
    _checkAndAddSubscribeProfile();
  }

  Future<void> _checkAndAddSubscribeProfile() async {
    final config = Provider.of<Config>(context, listen: false);
    if (config.subscribeUrl != null && config.profiles.isEmpty) {
      final newProfile = Profile.normal(label: "订阅配置", url: config.subscribeUrl!);
  ...
  Future<void> _updateProfiles() async {
    ...
    if (messages.isNotEmpty) {
      globalState.showMessage(
        ...
        message: TextSpan(
          children: [for (final message in messages) TextSpan(text: message, style: titleMedium)],
        ),
      );
    }
  }
...
class ProfileItem extends StatelessWidget {
  ...
  _handleDeleteProfile(BuildContext context) async {
    final res = await globalState.showMessage(
      title: appLocalizations.tip,
      message: const TextSpan(text: "确定要删除此配置文件吗？"),
    );
    if (res != true) return;
  ...
  _handleExportFile(BuildContext context) async {
    ...
    final res = await commonScaffoldState?.loadingRun<bool>(() async {
      ...
      final value = await picker.saveFile(profile.label ?? profile.id, file.readAsBytesSync());
      return value != null;
    }, title: appLocalizations.tip);
...
class ReorderableProfiles extends StatefulWidget {
  ...
  const ReorderableProfiles({super.key, required this.profiles});
