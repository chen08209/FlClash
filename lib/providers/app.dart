...
class DelayDataSource extends _$DelayDataSource with AutoDisposeNotifierMixin {
  ...
  /// Copies each touched URL's map once per call, so a bulk test must land in
  /// batches: a copy per result makes a run over n nodes O(n²).
  void setDelays(Iterable<Delay> delays) {
    final changed = <String, Map<String, int?>>{};
    for (final delay in delays) {
      final current = changed[delay.url] ?? state[delay.url];
      if (current?[delay.name] == delay.value) {
        continue;
      }
      changed.putIfAbsent(delay.url, () => {...?state[delay.url]})[delay.name] =
          delay.value;
    }
