...
class _ScriptsViewState extends ConsumerState<ScriptsView> {
  ...
  Future<void> _handleUpdate(Script script) async {
    await globalState.safeRun(
      () => ref.read(scriptsActionProvider.notifier).updateScript(script),
      title: script.label,
    );
  }
  ...
  Future<void> _handleEditorSave(
    ...
    final newScript =
        await (script?.copyWith(label: label) ?? Script.create(label: label))
            .save(content);
    ref.read(scriptsActionProvider.notifier).putScript(newScript);
    if (mounted) {
  ...
  Future<void> _handleImport(
    ...
    ref.read(scriptsActionProvider.notifier).putScript(script);
  }
  ...
  Future<void> _handleImportFromFile() async {
    ...
    final content = utf8.decode(await file.readBytes());
    if (!mounted) {
      return;
    }
