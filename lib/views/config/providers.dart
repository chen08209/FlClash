...
class _ClashProvidersViewState extends ConsumerState<ClashProvidersView> {
  ...
  void _put(ClashProvider provider, {ClashProvider? previous}) {
    ref
        ...
        .putProvider(provider, previous: previous);
  }
  ...
  Future<void> _handleToEditor([ClashProvider? provider]) async {
    final raw = (await provider?.content) ?? _template;
    if (!mounted) {
      return;
    }
    unawaited(
  ...
  Future<void> _handleDelete(ClashProvider provider) async {
    ...
    ref.read(clashProvidersActionProvider.notifier).delProvider(provider);
  }
