...
Future<bool> _handleSaveCustomProxy(BuildContext context, WidgetRef ref) async {
  ...
  try {
    await Future.wait([
      ref.read(customProxiesProvider(profileId).future),
      ref.read(proxyGroupsProvider(profileId).future),
      ref.read(clashConfigProvider(profileId).future),
    ]);
  } catch (_) {}
  if (!context.mounted) {
    return false;
  ...
  final issues = customProxyIssues(
    ...
    proxyGroups: ref.read(effectiveProxyGroupsProvider(profileId)),
  );
  if (issues.isEmpty) {
    try {
  ...
  Widget build(BuildContext context) {
    ...
    final issues = [
      ...customProxyIssues(
        ...
        proxyGroups: ref.watch(effectiveProxyGroupsProvider(profileId)),
      ),
