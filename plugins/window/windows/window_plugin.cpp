...
std::optional<LRESULT> WindowPlugin::HandleWindowProc(HWND hwnd,
                                                       ...
                                                       WPARAM wparam,
                                                       LPARAM lparam) {
  // Plugins register before the runner parents the Flutter view, so the
  // top-level window is only known once its messages arrive here.
  if (!relaunch_messages_allowed_) {
    relaunch_messages_allowed_ = true;
    AllowRelaunchMessagesThroughUipi(::GetAncestor(hwnd, GA_ROOT));
  }
...
void WindowPlugin::HandleEnsureInitialized(const flutter::EncodableMap*,
  ...
  controller_.Initialize(::GetAncestor(view, GA_ROOT), view);
  result->Success();
}
