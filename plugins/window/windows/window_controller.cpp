...
void WindowController::Initialize(HWND hwnd, HWND view) {
  if (hwnd_ == hwnd) {
    return;
  }
  hwnd_ = hwnd;
...
int WindowController::TopBorderHeight() const {
  return style_ != nullptr ? style_->TopBorderHeight(hwnd_) : 0;
}
...
void WindowController::PlaceView() const {
  ...
  const LONG height = client.bottom > border ? client.bottom - border : 0;
  ::MoveWindow(view_, 0, border, client.right, height, TRUE);
}
...
std::optional<LRESULT> WindowController::HandleWindowMessage(
  ...
  if (wparam && message == WM_NCCALCSIZE) {
    ...
    // Windows 11 draws the top border only into a non-client strip. Windows
    // 10 must get none: DWM draws the whole caption over a strip shorter than
    // one (Windows Terminal, NonClientIslandWindow::_UpdateFrameMargins).
    rect->top +=
        IsMaximized() ? frame.bottom : (IsWindows11OrGreater() ? 1 : 0);
    return 0;
    ...
    if ((pos->flags & SWP_FRAMECHANGED) != 0) {
      PlaceView();
    }
    ...
    RECT strip = paint.rcPaint;
    if (strip.bottom > border) {
      strip.bottom = border;
    }
    // DWM shows the extended frame where the client is painted black (alpha 0).
    if (strip.top < strip.bottom) {
      ::FillRect(dc, &strip,
    ...
    if (hit != HTCLIENT || point.y >= window.top + border) {
      return hit;
    }
    RECT frame{};
    ::AdjustWindowRectExForDpi(&frame, WS_OVERLAPPEDWINDOW, FALSE, 0,
                               ::GetDpiForWindow(hwnd));
    const LONG corner = -frame.left * 2;
    if (point.x < window.left + corner) {
      return HTTOPLEFT;
    }
    if (point.x >= window.right - corner) {
      return HTTOPRIGHT;
    }
    return HTTOP;
