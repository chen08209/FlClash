...
std::optional<LRESULT> WindowController::HandleWindowMessage(
  ...
  if (wparam && message == WM_NCCALCSIZE) {
    ...
    // DefWindowProc insets the resize borders for the window's DPI; only the
    // caption inset at the top is undone. Maximized windows hang past the
    // monitor edge by one frame width, so that much stays.
