...
bool ExtendsTopFrame(const WindowStyle& style) {
  return style.title_bar_style == TitleBarStyle::kHidden &&
         style.effect == Effect::kNone && !IsWindows11OrGreater();
}
...
void WindowStyle::Apply(HWND hwnd) const {
  ...
  // Windows fills an extended frame opaque, which would cover an accent effect.
  MARGINS margins{};
  if (effect == Effect::kMica) {
    margins = {-1, -1, -1, -1};
  } else if (ExtendsTopFrame(*this)) {
    RECT frame{};
...
int WindowStyle::TopBorderHeight(HWND hwnd) const {
  if (hwnd == nullptr || !ExtendsTopFrame(*this) || ::IsZoomed(hwnd)) {
    return 0;
  }
  const bool sizable =
